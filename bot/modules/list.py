...
def list_buttons(update, context):
    ...
    try:
        ...
        return sendMessage('𝐒𝐞𝐧𝐝 𝐚 𝐬𝐞𝐚𝐫𝐜𝐡 𝐤𝐞𝐲 𝐚𝐥𝐨𝐧𝐠 𝐰𝐢𝐭𝐡 𝐜𝐨𝐦𝐦𝐚𝐧𝐝 🙄', context.bot, update)
...
def select_type(update, context):
    ...
    if user_id != int(data[1]):
        ...
        list_method = data[3]
        item_type = data[2]
        editMessage(f"<b>𝐒𝐞𝐚𝐫𝐜𝐡𝐢𝐧𝐠 𝐟𝐨𝐫 <i>{key}</i> Please Wait...</b>", msg)
        Thread(target=_list_drive, args=(key, msg, list_method, item_type)).start()
    else:
        query.answer()
        editMessage("𝐥𝐢𝐬𝐭 𝐡𝐚𝐬 𝐛𝐞𝐞𝐧 𝐜𝐚𝐧𝐜𝐞𝐥𝐞𝐝!👀", msg)
...
def _list_drive(key, bmsg, list_method, item_type):
    ...
    if button:
        ...
        editMessage(f'😐 𝐍𝐨 𝐫𝐞𝐬𝐮𝐥𝐭 𝐟𝐨𝐮𝐧𝐝 𝐟𝐨𝐫 <i>{key}</i>\nUse better keywords', bmsg)
